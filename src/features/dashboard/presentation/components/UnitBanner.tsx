...
export function UnitBanner({ unitNumber, title, description, moduleSlug }: UnitBannerProps) {
  ...
  return (
    <div
      className={`w-full max-w-xl rounded-2xl overflow-hidden cursor-pointer hover:brightness-110 focus:outline-none focus-visible:outline-none module-${moduleSlug} ${isActive ? 'module-active' : ''}`}
    ...
    >
      <div className="flex items-center h-28 px-6">
        <div className="flex-1">
          <p className="text-md font-bold uppercase tracking-wider mb-1 text-primary-button opacity-60">
            SEÇÃO {unitNumber} • {description || title}
          </p>
          <p className="text-2xl font-bold text-primary-button">
            {title}
          </p>
        </div>
